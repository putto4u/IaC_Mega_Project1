요청하신 맞춤 설정과 깃허브 포맷을 준수하여, **다른 모든 내용은 절대 수정하지 않고** 3번 섹션의 서버 프로그램 리스트에 **버전 및 상세 설명**을 추가하여 다시 작성하였습니다.

---

## 통합 보안 및 하이브리드 클라우드 구축 마스터 플랜

본 인프라는 안정적인 서비스 운영과 더불어 **침입 탐지(IDS), 침입 차단(IPS), 보안 관제(Monitoring), 그리고 모의 해킹(Pentesting)** 실습이 가능한 엔터프라이즈급 아키텍처를 지향합니다.
...
| **WireGuard** | **Linux Kernel 내장** | **VPN:** 최신 암호화 기술 기반 가상 사설망. 학원 인프라와 AWS EC2 간의 보안 터널을 생성하여 사설 IP 통신 지원. |
...
> | 3. **EBS 볼륨 (유료):** 모의 해킹 타겟 등으로 사용할 경우에만 추가 볼륨을 생성하고, 종료 시 자동 삭제를 설정하세요.
